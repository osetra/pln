 ...
 * Зачем: toggle статуса одной клавишей без открытия меню.
...
export function createBatchHandlers(state, render) {
  return {
    ...
    'return': async () => {
      await toggleCurrentTaskStatus(state, render)
    },

    /** Toggle статуса текущей задачи через e (альтернативный биндинг) */
    'e': async () => {
      await toggleCurrentTaskStatus(state, render)
    },
  }
